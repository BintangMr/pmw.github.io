    ...
    <div class="bagian-kanan">
        <div class="info-film">
            ...
            foreach ($studios as $studio) {
                ?>
                <h2 class="text-center mt-5 fw-bold now-play-title" style="color: #ffff;"><?= $studio ?></h2>
                <?php
                foreach ($moviesTayang as $movie) {
                    $studioPembanding = str_replace(" ", "", $studio);
                    $studioMovie = str_replace("[", "", $movie[12]);
                    $studioMovie = str_replace("]", "", $studioMovie);
                    $studioMovie = str_replace('"', "", $studioMovie);
                    if (in_array($studioPembanding, explode(',', $studioMovie))) {
                        ?>
